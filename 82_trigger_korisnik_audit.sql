/* ==========================================================
   AUDIT TRIGGER: dbo.Korisnik_TR_LogChanges
   ...
   Proslijediti ID odgovornog korisnika prije DELETE naredbe:
     SELECT [log_IdResponsible] = @KorisnikId INTO #Korisnik_TR_LogChanges
   ========================================================== */
...
BEGIN
    RAISERROR(N'Tablica dbo.Korisnik ne postoji. Pokreni FAZU 1 (10_schema_cleanup.sql).', 16, 1);
    SET NOEXEC ON;
END;
GO
...
BEGIN
    RAISERROR(N'Tablica dbo.log_Korisnik ne postoji. Pokreni FAZU 1B (11_audit_log_tables.sql).', 16, 1);
    SET NOEXEC ON;
END;
GO

PRINT N'Kreiranje triggera: dbo.Korisnik_TR_LogChanges';
GO

/* ====================================================
   dbo.Korisnik_TR_LogChanges
==================================================== */
CREATE OR ALTER TRIGGER dbo.Korisnik_TR_LogChanges
ON dbo.Korisnik
...
BEGIN
    ...
    BEGIN
        IF OBJECT_ID('tempdb..#Korisnik_TR_LogChanges') IS NOT NULL
            SELECT @ResponsiblePersonId = t.log_IdResponsible FROM #Korisnik_TR_LogChanges t
    ...
    CREATE TABLE #korisnik_changed (admlog_row_number INT PRIMARY KEY, changed_columns VARCHAR(1500))
    IF @TriggerEvent = 3
    BEGIN
        INSERT INTO #korisnik_changed (admlog_row_number, changed_columns)
        SELECT
            i.admlog_row_number,
            changed_columns = SUBSTRING(''
                + CASE WHEN i.[Ime]           = d.[Ime]           OR ISNULL(i.[Ime],           d.[Ime])           IS NULL THEN '' ELSE ',Ime'           END
                + CASE WHEN i.[Email]         = d.[Email]         OR ISNULL(i.[Email],         d.[Email])         IS NULL THEN '' ELSE ',Email'         END
                + CASE WHEN i.[IsActive]      = d.[IsActive]      OR ISNULL(i.[IsActive],      d.[IsActive])      IS NULL THEN '' ELSE ',IsActive'      END
    ...
    END
    IF @debug = 1 SELECT [#korisnik_changed] = 'DEBUG', * FROM #korisnik_changed
    IF @TriggerEvent = 3 AND NOT EXISTS (SELECT 1 FROM #korisnik_changed WHERE changed_columns <> '') RETURN

    -- DELETE i staro stanje UPDATE-a
    IF @TriggerEvent IN (2, 3)
    BEGIN
        INSERT INTO dbo.log_Korisnik
            (KorisnikId, Ime, Email, IsActive, ResponsibleId, IsDeleted, DeletedAt,
             log_DateTS, log_IdResponsible, log_Action, log_ColumnsUpdated)
        SELECT
            d.[KorisnikId], d.[Ime], d.[Email], d.[IsActive], d.[ResponsibleId], d.[IsDeleted], d.[DeletedAt],
        ...
        LEFT JOIN #korisnik_changed ch ON ch.admlog_row_number = d.admlog_row_number
    END
    ...
    BEGIN
        INSERT INTO dbo.log_Korisnik
            (KorisnikId, Ime, Email, IsActive, ResponsibleId, IsDeleted, DeletedAt,
             log_DateTS, log_IdResponsible, log_Action, log_ColumnsUpdated)
        SELECT
            i.[KorisnikId], i.[Ime], i.[Email], i.[IsActive], i.[ResponsibleId], i.[IsDeleted], i.[DeletedAt],
            @Now, @ResponsiblePersonId,
            CASE @TriggerEvent WHEN 1 THEN 'I' WHEN 3 THEN 'UI' END,
            ch.changed_columns
        FROM (SELECT admlog_row_number = ROW_NUMBER() OVER (ORDER BY (SELECT NULL)), * FROM inserted) i
        LEFT JOIN #korisnik_changed ch ON ch.admlog_row_number = i.admlog_row_number
    END
END
GO

EXEC sp_settriggerorder @triggername = N'dbo.Korisnik_TR_LogChanges', @order = N'Last', @stmttype = N'DELETE';
EXEC sp_settriggerorder @triggername = N'dbo.Korisnik_TR_LogChanges', @order = N'Last', @stmttype = N'INSERT';
EXEC sp_settriggerorder @triggername = N'dbo.Korisnik_TR_LogChanges', @order = N'Last', @stmttype = N'UPDATE';

PRINT N'Trigger dbo.Korisnik_TR_LogChanges kreiran.';
GO
